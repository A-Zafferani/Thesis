...
def read(filename):
    ...
    dR3 = pd.read_csv(filename, sep=',', skiprows=(0), usecols=[3])
    R3 = np.array(dR3).transpose()[0]

    dR6 = pd.read_csv(filename, sep=',', skiprows=(0), usecols=[4])
    R6 = np.array(dR6).transpose()[0]

    dR5 = pd.read_csv(filename, sep=',', skiprows=(0), usecols=[5])
    R5 = np.array(dR5).transpose()[0] 

    return (A,R2,R1,R3,R6,R5)
...
A,R2,R1,R3,R6,R5 = read("Arestot_b copy.csv")
...
print('Resistance3=', param_3[0]-param_5[0]+(param_3[1]-param_5[1]), 'Ohm*um^2')
print('Resistance6=', param_6[0]-param_5[0]+(param_6[1]-param_5[1]), 'Ohm*um^2')

#dose for the wafers 
dose=np.array([3,7.2,115.2,230.4])
rdose = dose**0.5
...
plt.plot(rdose,intercept1+slope1*rdose, label='25$\mu m^2$')
...
eR2,eR1,eR3,eR6,eR5,n = read('errortot_b.csv')
...
parame_1, parame_cov_1 = opt.curve_fit(lin, rdose, mat[:,7], de1,err[:,7])
parame_2, parame_cov_2 = opt.curve_fit(lin2, rdose, mat[:,7], de2,err[:,7])
...
plt.figure(5)
plt.errorbar(rdose, mat[:,7], yerr=err[:,7], fmt='o', markersize=5)
...
dose2=np.array([0,3,7.2,115.2,230.4])
rdose2=dose2**0.5

de3=[intercept1,slope1]
de4=[1]

parame_3, parame_cov_3 = opt.curve_fit(lin, rdose2, mat2[:,2], de3,err2[:,2])
parame_4, parame_cov_4 = opt.curve_fit(lin2, rdose2, mat2[:,2], de4,err2[:,2])
print('parame3=',parame_3)
print('parame3 cov=',parame_cov_3)
print('parame4=',parame_4)
plt.figure(6)
plt.errorbar(rdose2, mat2[:,2], yerr=err2[:,2], fmt='o', markersize=5)
